      ...
      <div className="space-y-6">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-[#010009] border border-zinc-800/50 rounded-2xl p-6 animate-pulse h-40" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {statCards.map((stat, index) => (
              <div
                key={index}
                className="bg-[#1a1a2e]/50 border border-purple-500/10 rounded-2xl p-6 hover:bg-[#1a1a2e]/70 hover:border-purple-500/20 transition-all duration-200 cursor-pointer group"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-[#1a1a2e]/50 border border-purple-500/10 rounded-2xl p-6 hover:border-purple-500/20 transition-colors">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-500 text-xs mt-1">Daily chats — last 7 days</p>
              </div>
            </div>
            {loading ? (
              <div className="h-64 flex items-end justify-around gap-2">
                {[1, 2, 3, 4, 5, 6, 7].map((i) => (
                  <div key={i} className="flex-1 h-full bg-purple-500/5 rounded-t-lg animate-pulse" />
                ))}
              </div>
            ) : usageData.length === 0 ? (
                ...
                {usageData.map((item) => {
                  ...
                  return (
                    <div key={item.day} className="flex-1 flex flex-col items-center gap-2 group">
                      ...
                      />
                      <span className="text-xs text-gray-500 font-medium truncate max-w-full px-1">
          ...
          <div className="bg-[#1a1a2e]/50 border border-purple-500/10 rounded-2xl p-6 hover:border-purple-500/20 transition-colors">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-lg font-semibold text-white">Top Conversations</h2>
                <p className="text-gray-500 text-xs mt-1">Most recent chat topics</p>
              </div>
            </div>
            {loading ? (
              <div className="space-y-4">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="h-10 bg-purple-500/5 rounded-lg animate-pulse" />
                ))}
              </div>
            ) : topChats.length === 0 ? (
                    ...
                    <div key={index} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <p className="text-sm font-medium text-white truncate max-w-[180px]">{item.question}</p>
                        </div>
                        <p className="text-sm font-semibold text-white shrink-0">{item.count}</p>
                      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-[#1a1a2e]/50 border border-purple-500/10 rounded-2xl p-6 hover:border-purple-500/20 transition-colors">
            <div className="mb-4">
              ...
              <p className="text-gray-500 text-xs mt-1">Latest system events</p>
            </div>
            {loading ? (
                  ...
                  return (
                    <div key={item._id} className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-sm font-medium truncate">{item.content}</p>
                        <p className="text-gray-500 text-xs mt-0.5">{item.chatTitle}</p>
                        <p className="text-gray-600 text-xs mt-0.5">{timeAgo(item.createdAt)}</p>
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="bg-[#1a1a2e]/50 border border-purple-500/10 rounded-2xl p-6 hover:border-purple-500/20 transition-colors">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 hover:border-purple-500/30 rounded-xl p-6 transition-all group text-center"
              >
                ...
                <p className="text-white text-sm font-medium">Start New Chat</p>
              </button>
              <button
                onClick={() => router.push("/dashboard/conversations")}
                className="bg-gray-800/30 hover:bg-gray-800/50 border border-gray-700/30 hover:border-gray-600/50 rounded-xl p-6 transition-all group text-center"
              >
                <div className="w-12 h-12 mx-auto mb-3 rounded-lg bg-gray-700/30 flex items-center justify-center group-hover:bg-gray-700/50 transition-colors">
                  <MessageSquare size={24} className="text-gray-400" />
                </div>
                <p className="text-white text-sm font-medium">View Conversations</p>
              </button>
              <button
                onClick={() => router.push("/dashboard/analytics")}
                className="bg-gray-800/30 hover:bg-gray-800/50 border border-gray-700/30 hover:border-gray-600/50 rounded-xl p-6 transition-all group text-center"
              ...
              >
                ...
                <p className="text-white text-sm font-medium">Manage Settings</p>
              </button>
            </div>
          </div>
        </div>
